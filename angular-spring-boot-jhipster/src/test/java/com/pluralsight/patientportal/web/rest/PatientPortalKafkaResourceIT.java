...
import com.pluralsight.patientportal.config.KafkaProperties;
...
class PatientPortalKafkaResourceIT {
    ...
    void setup() {
        ...
        PatientPortalKafkaResource kafkaResource = new PatientPortalKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/patient-portal-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/patient-portal-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
